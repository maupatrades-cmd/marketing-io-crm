...
export default function SoldActionForm({ selected, user, onSuccess, onCancel }) {
  ...
  const rates = COMMISSION_RATES[assignedRole] || { setup: 0, monthly: 0 };
  const commSetup   = +(setupFee * rates.setup).toFixed(2);
  const commMonthly = +(monthly  * rates.monthly).toFixed(2);
  const commTotal   = commSetup + commMonthly;
    ...
    try {
      ...
      if (commSetup > 0) {
        await base44.entities.Commission.create({
          staff_id: assignedId, staff_name: assignedName, staff_role: assignedRole,
          commission_type: 'setup_commission',
          client_id: selected.id, client_name: selected.business_name,
          package_or_addon: pkg, base_amount: setupFee,
          rate_percent: rates.setup * 100, commission_amount: commSetup,
          qualifying_event: 'deal_closed_won', qualifying_event_date: today, status: 'pending',
        });
      }
      if (commMonthly > 0) {
        await base44.entities.Commission.create({
          ...
          client_id: selected.id, client_name: selected.business_name,
          package_or_addon: pkg, base_amount: monthly,
          rate_percent: rates.monthly * 100, commission_amount: commMonthly,
          qualifying_event: 'deal_closed_won', qualifying_event_date: today, status: 'pending',
        });
      }
      ...
      await base44.entities.ClientActivityLog.create({
        ...
        event_type: 'sales_sold', event_category: 'lead',
        event_summary: `Deal closed — ${pkgLabel} by ${assignedName}`,
        logged_by: user?.id || '', logged_by_name: user?.full_name || user?.email || 'Staff',
      });
  ...
  return (
    <div className="space-y-3 mt-2">
      ...
      <div className="rounded-lg bg-green-50 border border-green-200 px-3 py-2">
        <p className="text-xs font-semibold text-green-800 mb-1">Commission Preview ({(rates.setup * 100).toFixed(0)}% rate)</p>
        <div className="flex justify-between text-xs text-green-700">
          <span>On setup fee</span>
          <span className="font-medium">R {commSetup.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</span>
        </div>
        <div className="flex justify-between text-xs text-green-700">
          <span>On monthly</span>
          <span className="font-medium">R {commMonthly.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</span>
        </div>
        <div className="flex justify-between text-xs font-bold text-green-800 border-t border-green-200 mt-1 pt-1">
          <span>Total pending</span>
          <span>R {commTotal.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</span>
        </div>
        {commTotal === 0 && (
          <p className="text-xs text-green-600 mt-1 italic">No commission for {assignedRole} role.</p>
        )}
      </div>
