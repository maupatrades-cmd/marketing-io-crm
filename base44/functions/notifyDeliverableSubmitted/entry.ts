...
Deno.serve(async (req) => {
  try {
    ...
    if (prefs.email_deliverable_ready !== false) {
      try {
        await base44.asServiceRole.integrations.Core.SendEmail({
          to: client.email,
          from_name: 'Marketing iO',
          subject: `New Deliverable: ${deliverable.title}`,
          body: `
            <h2>${deliverable.title}</h2>
            <p>Hi ${client.contact_person},</p>
            <p>Your new deliverable <strong>${deliverable.title}</strong> is ready for review.</p>
            <p><a href="https://app.marketingio.co.za/client/deliverables" style="background: #a764e6; color: white; padding: 10px 20px; border-radius: 6px; text-decoration: none; display: inline-block;">Review Now</a></p>
    ...
    console.error('[notifyDeliverableSubmitted]', error);
