...
// Marketing iO director signature, embedded as a base64-encoded PNG with a
// transparent background. When present AND contract.signed_by_mio === true,
// the page-18 MIO signature box renders this image. When empty (legacy rows
// or signature asset not yet wired), the box falls back to the typed
// "/s/ Thapelo Maupa" notation — both forms are ECTA-valid e-signatures.
//
// To populate: take the cleaned (transparent-background, black-ink) PNG of
// Thapelo's signature, base64-encode it (`base64 -w 0 signature.png`), and
// paste the data-URL-stripped bytes here. Recommended dimensions ~ 600x200
// at 300dpi for crisp rendering inside the 80mm-wide signature box.
const MIO_SIGNATURE_PNG_BASE64 = '';
...
function pageExecution1(doc: any, ctx: MsaContext) {
  ...
  if (sigSignedByMio && MIO_SIGNATURE_PNG_BASE64) {
    try {
      doc.addImage(
        `data:image/png;base64,${MIO_SIGNATURE_PNG_BASE64}`,
        'PNG',
        ML + 1, y + 1, 70, 8,
      );
    } catch {
